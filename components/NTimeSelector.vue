<template>
  <div class="vtimeselector form-group">
    <label v-if="hasLabel" class="control-label">{{ label }}</label>
    <input
      type="text"
      readonly="readonly"
      autocomplete="off"
      class="vtimeselector__input form-control"
    ...
    <div class="vtimeselector__box" :class="{ 'vtimeselector__box--is-closed': pickerState.isClosed }">
      <ul class="vtimeselector__box__list vtimeselector__box__list--hours" v-if="displayHours">
        <li class="vtimeselector__box__head"><slot name="hours">Giờ</slot></li>
        <li
          class="vtimeselector__box__item vtimeselector__box__item--hours"
          v-for="(hour, index) in timeCount(interval.h, hoursLength)"
          :key="index"
          :class="{
            'timeselector__box__item--is-selected': pickerState.selected.hour == hour,
            'timeselector__box__item--is-highlighted': getState('hour', 'highlight', hour),
            'timeselector__box__item--is-disabled': getState('hour', 'disable', hour)
          }"
      ...
      <ul class="vtimeselector__box__list vtimeselector__box__list--minutes" v-if="displayMinutes">
        <li class="vtimeselector__box__head"><slot name="minutes">Phút</slot></li>
        <li
          class="vtimeselector__box__item vtimeselector__box__item--minutes"
          v-for="(minute, index) in timeCount(interval.m)"
          :key="index"
          :class="{
            'timeselector__box__item--is-selected': pickerState.selected.minute == minute,
            'timeselector__box__item--is-highlighted': getState('minute', 'highlight', minute),
            'timeselector__box__item--is-disabled': getState('minute', 'disable', minute)
          }"
...
export default class NTimeSelector extends Vue {
  ...
  @Prop({ type: Boolean, default: true }) displayHours!: boolean
  @Prop({ type: Boolean, default: true }) displayMinutes!: boolean
  @Prop({ type: Boolean, default: false }) displaySeconds!: Boolean
  @Prop({ type: String, default: ':' }) separator!: String
  @Prop({ type: Boolean, default: false }) initialView!: boolean
  @Prop({ type: Boolean, default: false }) padTime!: boolean
  @Prop({ type: Boolean, default: false }) form!: boolean
  @Prop(String) displayFormat!: String
  @Prop(String) returnFormat!: String
  @Prop({ type: Boolean, default: true }) h24!: boolean
  @Prop({ type: Boolean, default: false }) utc!: boolean
  valid: boolean = true
  interval = {
    h: 1,
    m: 10,
    ...
    return {
      ...
      hh: () => {
        if (this.h24)
          return this.pad(
            this.picker.hour > 12
              ? 12 - (this.picker.hour === 0 ? this.picker.hour + 1 : this.picker.hour)
              : this.picker.hour === 0
              ? this.picker.hour + 1
              : this.picker.hour,
            true
  ...
  get time() {
    ...
    if (!this.h24 && this.picker.ampm !== null) {
      const hour = parseInt(this.picker.hour.toString(), 10)
    ...
    return this.utc
      ? new Date(
          Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), this.picker.hour, this.picker.minute, this.picker.second)
        )
  ...
  selectTime(type, el, e) {
    ...
    if (!this.disabled && !this.getState(type, 'disable', el)) {
       ...
       * @type {String}
       */
      if (this.returnFormat) this.$emit('formatedTime', this.timeFormated('returnFormat'))
  ...
  setValue() {
    ...
    if (!this.pickerState.isPristine) {
      ;(this.pickerState.selected.hour = this.h24
        ? this.picker.hour
        : this.picker.hour > 12
        ? this.picker.hour - 12
        : this.picker.hour),
  ...
  created() {
    ...
    this.value !== null &&
      ['hour', 'minute', 'second'].forEach(
        type => (this.pickerState.selected[type] = this.picker[type] && this.pad(this.picker[type]))
      )

    // Get 24h again for input datetime format
    if (!this.h24 && this.value) this.picker.hour = firstHour

    // Warn if possible AM-PM confusion on selection
    if (
      (!this.h24 && (this.highlight.h && this.highlight.h.length > 0)) ||
      (this.disable.h != null && this.disable.h.length > 0)
    ) {
      console.warn(
        'You shouldn\'t use h24="false" with highlight or disable hour props. It may cause AM-PM confusion or limitation due to multiple hours selected.'
      )
    }
